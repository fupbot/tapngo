...
* **Port 80 is a default, not a hardcoded assumption.** `docker-compose.yml` publishes the frontend on `${FRONTEND_PORT:-80}:80` — zero-config for the common case, but a one-variable escape hatch (`FRONTEND_PORT=8080 docker compose up --build`) for anyone whose machine already has something on port 80, which is a real, common collision (a local Apache/nginx, another project's dev server). Without this, that person's `docker compose up --build` fails outright with a "port is already allocated" error and there'd be no way to reach the app at all short of editing the compose file.
